...
export function ComboBox(p) {
    ...
    //   }
    // };
    return (<KCombobox.Root options={p.options} optionValue={p.optionValue} optionLabel={p.optionLabel} optionTextValue={p.optionLabel} optionDisabled={p.optionDisabled} value={p.value} onChange={p.onChange} 
    // onInputChange={onInputChange}
    placeholder={p.placeholder} virtualized={false} disallowEmptySelection={false} itemComponent={function (props) { return (<KCombobox.Item item={props.item} class="hover:bg-gray-200 flex">
          <KCombobox.ItemLabel>{props.item.textValue}</KCombobox.ItemLabel>
          <KCombobox.ItemIndicator>
            <IconCheck />
          </KCombobox.ItemIndicator>
        </KCombobox.Item>); }}>
      <KCombobox.Control aria-label="Fruit" class="inline-flex items-center  border-2 border-base-300">
